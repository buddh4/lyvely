...
<template>
  ...
  <list-page title="account.my_account.info.emails" icon="email">
    ...
    <div v-for="userEmail in userEmails" :key="userEmail.email" class="flex py-4 px-3 bg-main items-center border-divide">
      <ly-icon v-if="userEmail.email === user.email" name="star" class="mr-1 text-pop"  title="account.my_account.info.main_email" tabindex="0" />
      <ly-icon v-else-if="!userEmail.verified" name="warning" class="mr-1 text-warning"  title="account.my_account.info.unverified_email" tabindex="0" />
      <ly-icon v-else  name="success" class="mr-1 text-success-light" title="account.my_account.info.verified_email" tabindex="0" />
      {{ userEmail.email }}
    </div>
  </list-page>
