...
const menuItemClass = 'block py-3 px-3 no-underline cursor-pointer flex no-wrap items-center h-12';
      ...
      <ul>
        <li>
          <router-link to="/" :class="menuItemClass">
            <ly-icon name="account" /> {{ $t('account.drawer.myAccount') }}
          </router-link>
        </li>
        <li>
          <router-link to="/" :class="menuItemClass">
            <ly-icon name="security" /> {{ $t('account.drawer.security') }}
          </router-link>
        </li>
        <li>
          <a :class="menuItemClass" @click="logout">
            <ly-icon name="logout" /> {{ $t('users.logout') }}
          </a>
        </li>
      </ul>
    </nav>
