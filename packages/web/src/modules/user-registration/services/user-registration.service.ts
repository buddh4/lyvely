...
export class UserRegistrationService implements IUserRegistrationService {
  async register(model: UserRegistrationDto): Promise<OtpInfo> {
    return unwrapEndpointRequest(registerRepository.register(model)).then((otpInfo) => new OtpInfo(otpInfo));
  }
  ...
  async resendVerifyEmail(dto: ResendOtpDto): Promise<OtpInfo> {
    return unwrapEndpointRequest(registerRepository.resendVerifyEmail(dto)).then((otpInfo) => new OtpInfo(otpInfo));
  }
}
