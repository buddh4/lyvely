...
<template>
  ...
  <div
    v-if="props.modelValue" class="min-w-screen
    h-screen animated
    fadeIn faster
    fixed
    left-0 top-0 flex justify-center items-center inset-0
     z-50 outline-none focus:outline-none bg-no-repeat bg-center
     bg-cover overflow-y-auto" tabindex="-1">

    <div class="fixed bg-black opacity-50 inset-0 z-0"></div>
      <div v-if="props.modelValue" class="w-full max-w-lg absolute mx-auto md:rounded-sm shadow-lg bg-white top-0 md:top-1/4 h-full md:h-auto">
      <div>
        <div class="flex items-center px-5 pt-5 pb-3 rounded-t-sm bg-white" data-modal-header>
          <slot name="header">
            ...
            <h5 class="text-lg inline-block align-middle flex align-items-center">
              <Icon v-if="icon" class="w-6 mr-2" :name="icon" :class="props.iconClass" :color="props.iconColor"></Icon>
              {{ $t(props.title) }}
            </h5>
          </slot>

          <Button v-if="props.submitButton" data-modal-submit class="primary rounded-full text-xs float-right align-middle ml-auto md:invisible px-2 py-0.5" @click="$emit('submit')">
