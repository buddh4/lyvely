...
export const useActivityStore = defineStore("activities", () => {
  ...
  const filter = ref(
    new ActivityFilter({ tagProvider: () => profileStore.profile?.tags || [] })
  );
  ...
  async function loadActivities() {
    ...
    const intervalFilter = tidCache.value.getDataPointIntervalFilter(
  ...
  async function move(
    ...
    try {
      ...
      const attachTo =
        moveEvent.newIndex > 0 ? to[moveEvent.newIndex - 1] : undefined;
