...
interface EmitterWrapper<Events extends Record<EventType, unknown>> {
  emitter: Emitter<Events>;
}

export type DelegateEmitterType<Events extends Record<EventType, unknown>> = EmitterWrapper<Events> & Emitter<Events>
...
export function useAsEmitter<Events extends Record<EventType, unknown>>(): Emitter<Events> {
  ...
  function on<Key extends keyof Events>(type: Key | '*', handler:  Handler<Events[Key]> | WildcardHandler<Events>): void {
  ...
  function off<Key extends keyof Events>(type: Key | '*', handler?: Handler<Events[Key]> | WildcardHandler<Events>): void {
  ...
  function emit<Key extends keyof Events>(type: undefined extends Events[Key] ? Key : never, event?: Events[Key]): void {
