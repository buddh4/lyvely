import { ContentFilter, ContentMetadataModel } from '../index';
...
describe('Content Filter', () => {
    ...
    describe('archived filter', function () {
        it('archived entries are excluded by default', async () => {
            ...
            const result = filter.check(new TestContent({ meta: new ContentMetadataModel({ archived: true }) }));
            expect(result).toEqual(false);
        });
        ...
        it('archive filter does not include non archived content', async () => {
            const filter = new ContentFilter({ archived: true });
            const result = filter.check(new TestContent({ meta: new ContentMetadataModel({ archived: false }) }));
            expect(result).toEqual(false);
        });
    });
});
