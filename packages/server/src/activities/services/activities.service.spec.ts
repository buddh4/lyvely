...
describe('ActivityService', () => {
  ...
  let HabitDataPointModel: Model<NumberDataPoint>;
  ...
  beforeEach(async () => {
    ...
    HabitDataPointModel = testingModule.get<Model<NumberDataPoint>>('DataPointModel');
  ...
  describe('sort', () => {
    ...
    it('sort to top', async () => {
      ...
      await activitiesService.sort(profile, user, habits[3], habits[3].timeSeriesConfig.interval);

      const filter = new DataPointIntervalFilter(ActivityTestDataUtil.getDateTomorrow());
      const { models: activities } = await activitiesService.findByFilter(profile, user, filter);
  ...
  describe('Activities service', () => {
    describe('findByRangeFilter', () => {
      ...
      it('do not find habit log outside of filter range', async () => {
        const { user, profile } = await testData.createUserAndProfile('user1');
        const habit = await testData.createHabit(user, profile);
        await testData.createDataPoint(
          user,
          profile,
          habit,
          ActivityTestDataUtil.getDateTomorrow(),
        );
    ...
    describe('findByRangeFilter', () => {
      ...
      it('find activity by profile and string id', async () => {
        const { user, profile } = await testData.createUserAndProfile('user1');
        const habit = await testData.createHabit(user, profile);
        const search = await activitiesService.findByProfileAndId(
          profile,
          assureStringId(habit._id),
        );
        expect(search).toBeDefined();
        expect(search.id).toEqual(habit.id);
      });
