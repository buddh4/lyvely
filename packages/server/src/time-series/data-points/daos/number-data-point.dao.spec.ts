...
import { TestTimeSeriesContent } from '../../test/test-time-series-content.schema';
import {
...
describe('NumberDataPointDao', () => {
  ...
  describe('quarterly interval', () => {
    ...
    it('do not include if interval level is daily', async () => {
      const { profile, uid, date } = await createEntity(
        '2022-02-20',
        CalendarIntervalEnum.Quarterly,
      );
      const result = await dao.findByIntervalLevel(
        profile,
        uid,
  ...
  describe('unscheduled interval', () => {
    ...
    it('do not include if interval level is daily', async () => {
      const { profile, uid, date } = await createEntity(
        '2022-02-20',
        CalendarIntervalEnum.Unscheduled,
      );
      const result = await dao.findByIntervalLevel(
        profile,
        uid,
