...
import { getAuthCookieName, getRefreshCookieName, getRefreshCookieExpiresIn } from '../guards/strategies';
...
export abstract class AbstractJwtAuthController {
  ...
  protected setAuthenticationCookie(req: Request, token: string) {
    ...
    const expirationMS = Math.max(
      ms(this.configService.get<string>('auth.jwt.access.expiresIn', DEFAULT_ACCESS_TOKEN_EXPIRES_IN)),
