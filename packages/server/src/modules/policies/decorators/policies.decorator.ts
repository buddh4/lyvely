...
export const Policies = (...handlers: Type<Policy<any>>[]) => SetMetadata(CHECK_POLICIES_KEY, handlers);

export function getPolicyHandlerFromContext(context: ExecutionContext, reflector: Reflector) {
  return reflector
    .getAllAndOverride<PolicyHandler<any>[]>(CHECK_POLICIES_KEY,  [context.getHandler(), context.getClass()]) || [];
}