...
export class TimerDataPointStrategy extends DataPointStrategy {
    ...
    prepareValue(config, value, oldValue) {
    ...
    prepareConfig(config) {
        ...
        if (isNumber(config.max) && config.max > 0 && config.max < 1000)
            config.max = TIMER_MAX_VALUE;
        if (isNumber(config.min) && config.min > 0 && config.min < 1000)
            config.min = TIMER_MAX_VALUE;
